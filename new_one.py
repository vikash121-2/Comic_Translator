    ...
    for image_path in image_paths:
        ...
        try:
            ...
            for (bbox_new, text_new, prob_new) in filtered_output:
                ...
                for i, existing_block_tuple in enumerate(final_text_blocks_raw):
                    ...
                    if len(text_new) > 0 and len(text_existing) > 0:
                        ...
                        if max(len(text_new), len(text_existing)) > 0:
                            text_similarity = matches / max(len(text_new), len(text_existing))

                    if iou > 0.5 and text_similarity > 0.7:
                        is_duplicate = True
            ...
            for (bbox_raw, text, prob) in final_text_blocks_raw:
                ...
                formatted_blocks.append({"text": text, "location": simple_bbox})
...
async def not_implemented_yet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles buttons for features that are not fully built out yet."""
    query = update.callback_query
...
async def process_collected_images(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ocr_data = get_ocr_results(image_paths, min_confidence=0.35)
